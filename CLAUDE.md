...
- `src/layout/Slide/SlideLayout.tsx` — the core screen. Fetches a page of images (`GET /image/me`, paginated envelope `{ images, limit, page, totalPages }`), walks through them one at a time (`currentIndex`), and `POST`s each labeling result to `/analysis` before advancing. Tracks `isFinished` (queue exhausted) and `goalDone` (last image reached and a goal exists) to drive the results screen (`SlideSessionResults`). The labeling form collects the full `CreateResultDto` — ten clinical enum fields defined in `src/lib/analysis/fields.ts`.
...
- `src/components/Slide/SlideLabel/*` — the labeling form. `SlideLabeling` is a bounded `snap-y snap-mandatory` scroll area rendering one `SlideFieldSection` per question; fields with more than six options use `SlideMultiSelectDropdown` (a portalled popover), the rest use inline pills. State is `AnalysisResultState`, threaded down as props — there's no separate slide-labeling store.
- `src/lib/analysis/*` — the analysis contract in one place: `types.ts` (enums mirroring the backend wire values), `labels.ts` (**all pt-BR copy — edit terminology here, not in components**), `fields.ts` (the descriptor array driving order, controls and validation), `state.ts` (pure toggles, including "none" exclusivity) and `validation.ts` (the completeness gate and payload mapping). `null` means unanswered; `[]` means the analyst explicitly answered "nothing found" on the two fields whose taxonomy has no enum for it.
