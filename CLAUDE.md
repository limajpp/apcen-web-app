...
- `src/routes/router.tsx` — route table. `/` redirects to `/login`; `/login` is wrapped in `PublicRoute` (signed-in users are sent to `/slide-analysis`); `/slide-analysis` is wrapped in `PrivateRoute`; unknown paths redirect to `/`.
...
- `src/layout/Slide/SlideLayout.tsx` — the core screen and its session state. Loads the queue with `fetchSlideQueue` (admins get `GET /image/conflict`, everyone else `GET /image/me`), walks it one slide at a time and saves each slide with `createSlideRecord` (`POST /verdict` for admins, `POST /analysis` otherwise). It keeps the last saved slide so the user can go back exactly one slide: the current form is kept as a draft, the edit is sent with `updateSlideRecord` (`PATCH /analysis/:id` or `/verdict/:id`), and a 409 means a verdict now exists and the edit is refused. The progress bar comes from the backend: the user's analysis count (`fetchCompletedCount`) against their current goal (`fetchCurrentGoal`, `GET /user/me`), capped by the slides they can still reach; when a save reaches the goal the notice shows and the raised goal is fetched again. The token's `goal` claim is not used. Only the current draft is persisted per user in `localStorage` (`@Apcen:slide-session:<userId>`) so a reload resumes it. Renders `AnalysisLayout` for the active screen and `BaseLayout` (with the side art) for the result and load-error screens. The labeling form collects the full `CreateResultDto` — ten clinical enum fields defined in `src/lib/analysis/fields.ts`.
- `src/components/Slide/SlideView/SlideViewer.tsx` — wraps OpenSeadragon for deep-zoom/pan of the slide image; imperative `zoomIn`/`zoomOut`/`reset` handle exposed via `forwardRef`/`useImperativeHandle`. Logs on `open-failed`, hides OpenSeadragon's raw message and shows a pt-BR error with a retry that rebuilds the viewer, and refuses to construct a viewer over an empty URL.
- `src/components/Slide/SlideLabel/*` — the labeling form. `SlideLabeling` is a bordered panel with one continuous scroll (`analysis-scroll` utility in `src/index.css`) holding the numbered questions as inline radio/checkbox pills. Only visible questions are rendered and counted in the badge: question 10 ("Possui interferência do artefato?") appears only when a technical artifact other than "Sem artefato" is marked. New slides start from `initialAnalysisResult` (`src/lib/analysis/types.ts`), with "Sem alteração", "Outro achado" and "Sem artefato" pre-marked on questions 7, 8 and 9. After a blocked submit, unanswered questions turn red with an asterisk. State is `AnalysisResultState`, threaded down as props — there's no separate slide-labeling store.
...
- `src/lib/analysis/*` — the analysis contract in one place: `types.ts` (enums mirroring the backend wire values), `labels.ts` (**all pt-BR copy — edit terminology here, not in components**), `fields.ts` (the descriptor array driving order, the `required` flag and exclusive values), `state.ts` (pure toggles, including exclusivity), `validation.ts` (the completeness gate and payload mapping) and `progress.ts` (progress stages and segment fills). Every visible question is required; a descriptor's `visibleWhen` hides it, and a hidden `artifactSeverity` is sent as `no_interference`. `null` means unanswered.
